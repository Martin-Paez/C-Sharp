...
namespace TP.TP3
{
    public class TpTres
    {
        ...
        {
            Func<bool>[] f = { exEjSiete };
            Menu.run(ref f,
                  "Ejercicios:              \n"
                + "-----------              \n"
                + " 1)Ejercicio 6 (tp1.ej7)  \n"
                + " s)Salir                 \n");
            return false;
        }
        public static bool exEjSiete()
        {
            Console.WriteLine("Ejercicio 7:\n-------------\n");
            Pila<Numero> p = new Pila<Numero>();
            Cola<Numero> c = new Cola<Numero>();
            Llenar(p);
            Llenar(c);
        ...
        public static void Llenar<T>(Coleccionable<T> c) where T : Comparable<T>
        {
            for (int i = 0; i < 20; i++)
                c.Agregar(FabricaDeComparables<T>.CrearAleatorio(100));
        }
